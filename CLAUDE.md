...
2. **Los límites de seguridad crónica** (vitamina D, yodo, selenio,
   ...
   cae y se dice. ⚠️ **Hoy no hay ningún suelo encendido**: el único escrito es
   la vitamina E del perro sano (67,1 mg/1000 kcal), y lleva
   `aplicado_por_el_solver: false` porque al encenderlo la batería dio seis
   fallos — deja sin menú al toy de 1,5 kg y al adulto con ocho especies fuera, y
   en varios perros mete comida que nadie pidió sin avisar. El perro normal sí lo
   cumple (68-79 mg en peldaño estricto); la causa es que **en el catálogo no hay
   un suplemento de vitamina E suelto**. La cifra no se baja: se queda escrita con
   su medida y se pregunta, que es la regla. En adulto son
...
| `leer_sacn5.py` + `lecturas_sacn5.json` | **El mismo contador, para el TEXTO de SACN5** (10 de septiembre). Su hermano para las tablas es `sacn5_tablas.json`. No cuenta todo: SACN5 es un libro de clínica y la mayoría de sus cifras son epidemiología o dosis de fármaco, así que el filtro es **nutricional y está escrito** —una frase entra si lleva cifra con unidad **y** nombra un nutriente, o si es una recomendación dietética que nombra uno—. Salen **3.140** elementos y hoy **ninguno** tiene veredicto. Ese número se compara **exacto** en el BLOQUE 81, igual que el 78 con las tablas: baja solo cuando alguien lee y lo baja en el mismo commit, y si alguien relajara el filtro para que bajase solo, el total cambia y salta |
...
`sacn5_tablas.json` + `auditar_sacn5_tablas.py` (10 de septiembre) son **el inventario de las 474 tablas de SACN5**, una por una, con lo que el motor hace con cada una. Nació de una pregunta de Elena: «¿leída completa significa leída de verdad, con todas sus tablas?». La respuesta era **no se puede comprobar**, que es justo lo que le había pasado a FEDIAF. Hoy: 64 con rastro en el repo, 44 felinas y 75 listados de productos —estas dos últimas apartadas **por su propio título**, y el campo `veredicto_por` lo dice—, 2 leídas enteras sin nada que aplicar, **3 leídas CON hallazgo** y **286 sin veredicto**. Los dos números —las pendientes y las que traen un hallazgo sin aplicar— los clava el BLOQUE 78 **exacto y por separado**: si se mezclaran, el que baja al trabajar taparía al que sube al encontrar algo. Los hallazgos van medidos en `HALLAZGOS_SACN5_10SEP.md`, y los dos más gordos son que SACN5 pide **cuatro veces** la vitamina E que damos al perro sano —en cinco capítulos distintos— y que su tabla de energía del cachorro (33-8, de NRC 2006) va hasta un **28 % por encima** de la curva de Klein que publica FEDIAF y que aplicamos.
