...
| `pruebas_completas.py` | **La batería.** Los 126 bloques, ~75 min. Es lo que se ejecuta entero antes de entregar cualquier cambio — y desde el 18 de septiembre tiene dos escalones por debajo, la rápida (1 min) y la repartida en cuatro (18 min): ver «Cómo se prueba» |
...
### Los atajos del catálogo servían un menú CRUDO cuando se pedía COCINADO

*(19 de septiembre de 2026.)* **CASO REAL, EN PRODUCCIÓN**, encontrado midiendo
cuánto tarda cada celda: pidiendo un menú **cocinado**, `/menu/v2` devolvía en
**0,1 s** un menú de fichas **crudas** —con `Carcasa de pollo` dentro, que es
hueso carnoso— y la respuesta decía `modo_de_preparacion: "cocinado"`.

**El hueso COCIDO astilla.** En cocinado no es «una proporción que cede»: es una
exclusión **dura**, como una alergia, y así lo dice el propio `GET /vocabulario`
citando SACN5 cap. 50 —46 de 60 cuerpos extraños esofágicos retirados a perros
eran hueso—.

⚠️ **La causa es la familia de fallo que este repo ya tiene escrita tres veces**:
el catálogo de menús fijos se generó el **5 de agosto**, el modo cocinado entró
el **18 de septiembre**, y **los tres atajos que lo usan no se volvieron a
mirar**. Es lo mismo que `motor_completo` avisa de las patologías —«si un camino
nuevo llama al motor, tiene que pasarle `patologias`»— con la diferencia de que
aquí el olvido **no se ve en ninguna cifra**: el semáforo comprueba los 43
requisitos, no de qué modo son las fichas. Medido con el guardia quitado: **5 de
5 perros** con variante en el catálogo servían hueso crudo en cocinado.

**Los tres atajos, y los tres lo tenían**: la vía rápida del primer menú (base
del catálogo forzada al solver), las variantes pre-resueltas de cualquier menú
de la sesión, y el atajo de Personalizar cuando solo se pide «todo el/la X».

⚠️ **Se pregunta por los ALIMENTOS, no por una etiqueta.** Podría haberse puesto
un `MODO_DEL_CATALOGO = "crudo"` y sería una frase más que se queda parada: el
día que se regeneren los menús para cocinado habría que acordarse de cambiarla.
`catalogo_menus.vale_en_este_modo()` mira las fichas una a una con
`accesibles.vale_en`, que es un **hecho** sobre el menú y sigue siendo verdad se
regenere lo que se regenere.

⚠️ **Y el cocinado pasa a costar lo que cuesta resolverlo**: 2,9 s aquí (~18 s
en Render) en vez de 0,1. Eso es correcto —antes era rápido porque estaba mal—
y es justo lo que arregla **regenerar el catálogo también para cocinado**, que
está medido y escrito en `PENDIENTE_PRODUCTO.md`.

Lo vigila el **BLOQUE 133**, y lo que exige no es el guardia sino el
**invariante**: ningún menú que la API entregue puede llevar un alimento que no
valga en el modo pedido. Con una segunda mitad que hace falta —que alguna
petición vaya **de verdad** por el catálogo (`via_catalogo`)—, porque el camino
normal siempre respeta el modo y sin atajos el bloque saldría verde sin probar
nada. Comprobado con el fallo puesto: 5 rojos.
...
Los **126 bloques** de la entera tardan unos **75 minutos**. Esa cifra sube cada
