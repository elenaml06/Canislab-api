...
def _seguridad_completa(gramos, al, der, etapa, patologias=None, peso_perro_kg=None):
    ...
    #
    #   «a lo mejor la persona que vaya a comprar al supermercado pide cerebro
    #    de ternera y dice: no tengo, pero tengo de vaca. Y problema.»
    #
    # El caso es real y el motor no podía evitarlo de ninguna forma: el menú
    # dice «Cerebro de ternera · 25 g» y en la carnicería te ofrecen sesos de
    # vaca como si fueran lo mismo. Y los de vaca son material especificado de
    # riesgo -- encéfalo de bovino de más de 12 meses, Reg. (CE) 999/2001
    # anexo V --, o sea que NO se pueden dar a un animal. El motor había hecho
    # su parte sacando esa ficha del catálogo, y eso NO BASTA: la sustitución
    # ocurre en el mostrador, donde el motor no está.
    #
    # ⚠️ Y SACAR LA FICHA EMPEORÓ ESE RIESGO EN VEZ DE MEJORARLO: antes estaban
    # las dos en la lista y la distinción se veía; ahora solo aparece «de
    # ternera» y quien la lea no tiene forma de saber que la otra no vale.
    #
    # Va por `problemas_seguridad` por lo mismo que el aviso de la grasa de la
    # pancreatitis, escrito doce líneas más arriba: es el canal que la app ya
    # pinta en TODOS los caminos (generar, semana, varios perros, editar,
    # revalidar), así que sale en los ocho sitios sin tocar la app.
    for _n_ac in sorted(gramos):
...
def _resolver_menu_v2_crudo(datos: PeticionMenu):
    ...
    if not ok:
        ...
        if _choque:
            ...
            # ⚠️ DOS MENSAJES, PORQUE SON DOS COSAS DISTINTAS (11 septiembre).
            # Con dos o más culpables hay un CHOQUE y quien firma elige cuál
            # cede. Con uno solo no hay nada que elegir: hay un límite que no
            # deja margen, y decir «chocan» de un límite solo sería afirmar
            # algo falso. Hasta hoy el caso de uno solo no se decía -- se caía
            # al mensaje genérico del dueño, que es justo lo que este
            # diagnóstico existe para no darle a un veterinario. Medido ese
            # día: renal + pancreatitis en un adulto de 25 kg tiene DIEZ
            # límites activos y solo la grasa de la pancreatitis desbloquea.
            if _choque.get("es_un_solo_limite"):
                _motivo_txt = (
                    "No hay ninguna ración que cumpla los límites de las patologías "
                    "marcadas. De todos los que se aplican, el único que no deja "
                    "margen es este: " + _texto + ". Soltándolo sí habría menú, y "
                    "soltando cualquiera de los demás no. Que ese límite ceda es una "
                    "decisión clínica, así que no la toma la app.")
            else:
                _motivo_txt = (
...
SELLOS_DE_LOS_DATOS = {
        ...
        "requerimientos_v2_final.json": "db402129ae887739",   # ⚠️ 13 sep (noche, 5): FILA NUEVA "Omega6_total", con los SEIS campos a "-" -- mismo patron que Fibra, Taurina, L_carnitina, EPA y Omega3_total: FEDIAF no pide omega-6 TOTALES en el perro (la Tabla III-3b trae el linoleico y el araquidonico por separado, nunca la suma), asi que no exige ni limita nada a un perro sano. Existe porque el ratio omega-6:omega-3 que SACN5 pide en cuatro patologias necesita los DOS lados y este lado no existia: el motor no podia ni medirlo ni ofrecerselo al profesional. Se calcula como linoleico + araquidonico CON FACTOR DE UNIDAD (el linoleico va en gramos y el araquidonico en miligramos: sumarlos a pelo da mil veces el araquidonico), y esa conversion la REHACE el BLOQUE 106. Comprobado con auditar_fediaf.py antes de mover este numero: 251 comprobaciones, 0 discrepancias.
...
# ⚠️ CUÁNTO CRUDO HAY QUE COMPRAR PARA ESOS GRAMOS COCIDOS (18 de septiembre
# de 2026), y es un fallo medido, no una mejora.
#
# Los gramos de un menú cocinado son de comida YA COCINADA —la ficha lo declara
# en `se_pesa` y el aviso al comprar lo repite— pero **en la tienda se compra
# CRUDO**. Y la diferencia no es un redondeo: medida sobre las 69 fichas que se
# pesan cocidas, va de **×0,42 a ×1,99**. Del pulpo hay que comprar el DOBLE de
# lo que dice el menú; del arroz, menos de la mitad. Una lista de la compra que
# no lo convierta manda a la tienda a por la cantidad equivocada, y en el peor
# caso por la mitad de la comida del perro.
#
# ⚠️ SE DERIVA DEL AGUA, que es lo que de verdad cambia al cocer, y no de una
# tabla de factores escrita a mano —que se quedaría parada con la primera ficha
# nueva y no daría ningún error. La materia seca es lo que se conserva:
#
#     gramos_crudos = gramos_cocidos × (materia seca del cocido / la del crudo)
#
# Y el crudo con el que se compara sale de DOS sitios, en este orden:
#   1. su ficha hermana CRUDA del catálogo, cuando existe («Zanahoria cocida» ->
#      «Zanahoria»), que es la que el propio motor usa en modo crudo;
#   2. la FILA CRUDA DE SU FUENTE, declarada en `auditar_cocinados.FILA_CRUDA`
#      —la misma que audita que la conversión no miente— para las que no tienen
#      hermana: el arroz, la patata, el cerdo…
#
# Lo que no se puede calcular **se queda sin cifra y se dice**. Y lo que sale de
# un ancla APROXIMADA va marcado como tal: una cifra aproximada presentada como
# exacta en una lista de la compra es exactamente lo que no puede pasar.
_CRUDO_POR_COCIDO = None


def _calcular_crudo_por_cocido(alimentos):
    """{ficha cocida: {factor, de_donde, aproximado}} — derivado, nunca escrito.

    El crudo con el que se compara sale de DOS sitios, en este orden:

      1. su ficha hermana CRUDA del catálogo, cuando existe («Zanahoria cocida»
         -> «Zanahoria»), que es la que el propio motor usa en modo crudo. Son
         58 de las 69, y no se copia nada: se lee su `humedad_g_100g`, o sea que
         el día que esa ficha cambie de agua, el factor cambia solo;
      2. el agua del crudo DECLARADA en la propia ficha
         (`humedad_crudo_g_100g` + `humedad_crudo_fuente`), para las que no
         tienen hermana — el arroz, la patata, el cerdo, el atún claro… Se
         declara con la fila literal de la que sale, que es lo que permite
         rehacerla, igual que `humedad_fuente`.

    Lo que no se puede calcular **se queda sin cifra**, y eso también es una
    respuesta: hoy le pasa a «Vaca para guisar cocida», porque la fila cruda que
    la ancla (ciqual:6231) NO publica agua. Inventarle el factor de otro corte
    de vaca sería poner una cifra en una lista de la compra sin fuente.
    """
    salida = {}
    for nombre, ficha in alimentos.items():
        if str(ficha.get("se_pesa") or "").strip().lower() != "ya cocido":
            continue
        agua_coc = ficha.get("humedad_g_100g")
        if agua_coc is None:
            continue
        ms_coc = 100.0 - float(agua_coc)
        base = nombre.rsplit(" ", 1)[0] if " " in nombre else None
        hermana = alimentos.get(base) if base else None
        agua_cru = de_donde = aprox = None
        if hermana is not None and hermana.get("humedad_g_100g") is not None:
            agua_cru = float(hermana["humedad_g_100g"])
            de_donde = f"la ficha cruda «{base}» del catálogo"
        elif ficha.get("humedad_crudo_g_100g") is not None:
            agua_cru = float(ficha["humedad_crudo_g_100g"])
            de_donde = ficha.get("humedad_crudo_fuente")
            aprox = ficha.get("humedad_crudo_aproximada")
        if agua_cru is None or agua_cru >= 100:
            continue
        ms_cru = 100.0 - agua_cru
        if ms_cru <= 0:
            continue
        salida[nombre] = {"factor": round(ms_coc / ms_cru, 2),
                          "de_donde": de_donde,
                          "aproximado": bool(aprox),
                          "por_que_aproximado": aprox}
    return salida
...
def _arbol_de_alimentos():
    ...
    global _RICO_EN, _CRUDO_POR_COCIDO
    if _RICO_EN is None:
        _RICO_EN = _calcular_rico_en({a["nombre"]: a for a in _ca()})
    if _CRUDO_POR_COCIDO is None:
        _CRUDO_POR_COCIDO = _calcular_crudo_por_cocido({a["nombre"]: a for a in _ca()})
    for a in _ca():
        ...
        # ⚠️ CUÁNTO CRUDO HAY QUE COMPRAR, junto al alimento (18 de septiembre
        # de 2026). Los gramos del menú son de comida YA COCINADA y en la
        # tienda se compra CRUDO: medido sobre las 69 fichas que se pesan
        # cocidas, el factor va de ×0,20 a ×1,99. Del pulpo hay que comprar el
        # DOBLE de lo que dice el menú y de los copos de avena una QUINTA
        # PARTE. Una lista de la compra sin esto manda a la tienda a por la
        # cantidad equivocada.
        _cru = (_CRUDO_POR_COCIDO or {}).get(a["nombre"])
        if _cru:
            fila["cuanto_crudo_hace_falta"] = _cru
        ...
        # escribirse).
        #
        # `aviso_al_comprar` nació el 13 de septiembre «para que salga por las
        # DOS puertas», y una de las dos estaba a medias: se servía en
        # `por_categoria` —que es la lista que lee el FORMULADOR DEL
        # VETERINARIO— y NO en `pantallas`, que es Personalizar, o sea la
        # pantalla donde el dueño elige la comida a mano.
        #
        # O sea que el aviso escrito para quien está delante del mostrador
        # —«si te ofrecen sesos de vaca, NO valen»— solo lo veía un
        # veterinario. Es la regla 6 por el lado que no se ve: el motor sirve
        # un texto perfecto y no lo lee nadie.
        if a.get("aviso_al_comprar"):
            fila["aviso_al_comprar"] = a["aviso_al_comprar"]
...
def listar_alimentos():
    ...
    for a in _ca():
        por_cat.setdefault(a["categoria"], []).append({
            "nombre": a["nombre"],
            "kcal_100g": a["energia"],
            # ⚠️ LA ESPECIE LA SABE EL MOTOR, y aquí se leía una clave que las
            # fichas NO tienen: `a.get("especie")` devolvía None en los 163.
            "especie": _esp(a["nombre"]),
            # ⚠️ La SEGUNDA puerta del aviso de compra (13 septiembre). La
            # primera es `problemas_seguridad`, que sale con el menú ya hecho;
            # esta sale ANTES, cuando alguien elige el alimento a mano en el
            # analizador o en Personalizar. Hacen falta las dos: con solo la
            # del menú, quien lo elige a mano no lee nada hasta el final.
            # Y va aquí y no copiado en la app porque el aviso es del MOTOR:
    ...
    return {
        ...
        # ⚠️ CÓMO SE MONTA EL PLATO ENTERO, que es lo que preguntó Elena el 17 de
        # septiembre —«¿se tritura todo junto y se da modo puré?»— y no se
        # contestaba en ningún sitio: había texto de cada alimento y de cada
        # categoría, y ninguno de lo que tiene delante quien va a cocinar.
        "como_se_prepara_el_plato": _COMO_SE_DA.get("como_se_prepara_el_plato", {}),
