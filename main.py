...
SELLOS_DE_LOS_DATOS = {
        ...
        "alimentos_v3_final.json":      "6c43b0094f0fca21",   # 8 sep (4): EL CATALOGO, DIRECCIONABLE. `fuentes_id` en 99 de las 163 fichas: el identificador de BEDCA, CIQUAL y/o USDA que le corresponde a cada una, aceptado SOLO si la fila candidata cuadra con la nuestra en proteina, grasa, agua y energia Y el nombre es compatible en preparacion y en especie. Ni un numero cambia -- comprobado, 0 casillas movidas: esto es indice, no dato. Existe porque el catalogo se habia revisado seis o siete veces y siempre se volvia a abrir, y la causa no eran los datos sino que NO ERA DIRECCIONABLE: 117 fichas sin identificador y las otras 45 con el escrito en prosa dentro de nota_datos, donde ninguna herramienta lo lee. Sin identificador hay que buscar por nombre, y buscar por nombre falla -- medido: "Atun" devuelve "Atun en aceite de oliva", "Salmon" devuelve "Queso para untar con salmon", "Cardo" devuelve "Anacardo". Lo pone y lo revisa `fijar_identificadores.py`. // 8 sep (3): LO QUE NO TIENE PROCEDENCIA INDIVIDUAL, A HUECO DECLARADO. 86 casillas rescatadas de la rama `nutrition-audit-data-validation-bihto9`, que nunca se fusiono. El patron que las delata es que el MISMO numero se repite entre fichas distintas: 21 cobres con solo cuatro valores en todo el grupo (0,08 en siete blancos, 0,72 en cinco cefalopodos, 0,1 en cinco, 0,05 en dos) y 70 celdas repartidas en seis huesos carnosos donde conejo, pato, pollo y cordero declaran la misma vitamina A (0,01), la misma vitamina D (0,01) y la misma riboflavina (0,2) siendo especies distintas. Una constante por familia no es una medida individual. COMPROBADO CONTRA LA FUENTE antes de vaciar: BEDCA no publica cobre para besugo, lubina, pulpo, calamar, trucha, lenguado ni pescadilla -- la celda no existe en la respuesta, asi que esos 0,08 no salen de ahi. El valor pasa a 0 y la clave entra en `sin_dato`, o sea que contra un TECHO deja de contar como cero medido y se imputa al percentil 90 de su familia (ver constructor.valor_para_maximo). MEDIDO antes de entregarlo: ninguna familia baja de los 3 donantes que pide MINIMO_FAMILIA, los 16 cobres de pescado SUBEN de 0,08 a 0,41 imputado, y CERO fichas empeoran -- no hay ni una que ya fuera hueco y se quede con un percentil mas bajo. Y 9 casillas mas encontradas por el detector nuevo de auditar_catalogo.py y COMPROBADAS CONTRA LA FUENTE una a una: el manganeso de ocho pescados blancos valia 0,035 en los ocho, que es el valor del CALAMAR en USDA -- para sus especies USDA da 0,011 (eglefino), 0,014 (lenguado), 0,015 (lubina) y 0,7 (perca), ninguno 0,035; a sin_dato los ocho, porque merluza no es eglefino y mapear a ojo es la trampa de los dos peces en una fila. Y la colina de la lubina, 65 -> 60,80 (USDA, Fish sea bass mixed species raw). LO QUE EL DETECTOR SENALO Y RESULTO SER CORRECTO, comprobado igual y anotado para no repetir el viaje: la colina 65 de 17 pescados la publica USDA asi para bacalao (65,2), eglefino, calamar, lenguado, perca, pulpo y trucha; el zinc 0,1 de 14 verduras lo da BEDCA con AR individual (manzana, pepino, alcachofa); y el yodo 1 de 10 verduras es ioduro=1 con AR en BEDCA, confirmado en siete de ellas por separado. Repetirse no prueba nada: hay que abrir la fuente. // 8 sep (2): 159 -> 163 fichas, dos trabajos que se hicieron a la vez y aqui se juntan. DEL PR #87: "Pets Purest Aceite de Salmon Escoces", con la foto de la etiqueta fisica del bote. DE ESTE: tres piezas que llevaban nombre de TERNERA y datos de VACA (timo, pulmon y cerebro -- coincidian celda a celda con los registros de vaca de USDA y no con los de ternera), partidas en dos para que cada especie tenga SUS datos; el timo de vaca tiene 236 kcal y 20,35 g de grasa y el de ternera 101 y 3,07. Mas las 52 celdas cerradas contra BEDCA/CIQUAL/USDA en el orden de Bases.md, ninguna de las cuales era un numero mal copiado: las 52 eran CEROS. auditar_catalogo.py y auditar_fediaf.py re-ejecutados, pruebas_completas.py entero.
