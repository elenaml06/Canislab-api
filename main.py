...
SELLOS_DE_LOS_DATOS = {
        ...
        "alimentos_v3_final.json":      "20a15984bafa669f",   # 8 sep (3): CORREGIDO. El sello declarado era "de79e5d5033cdbcd" y el SHA-256 real del fichero es "20a15984bafa669f" -- estaba mal en `main`, no lo rompio este trabajo (comprobado: `git show origin/main:alimentos_v3_final.json | sha256sum` da el real, y el diff del catalogo contra main esta vacio). O sea que el BLOQUE 12 llevaba en rojo en main y /verificar estaba diciendo en produccion que los datos no cuadran, que es exactamente para lo que existe ese endpoint. El cambio del catalogo SI fue a proposito (el comentario de abajo lo describe: 159 -> 163 fichas); lo que se declaro mal fue el hash.   # 8 sep (2): 159 -> 163 fichas, dos trabajos que se hicieron a la vez y aqui se juntan. DEL PR #87: "Pets Purest Aceite de Salmon Escoces", con la foto de la etiqueta fisica del bote. DE ESTE: tres piezas que llevaban nombre de TERNERA y datos de VACA (timo, pulmon y cerebro -- coincidian celda a celda con los registros de vaca de USDA y no con los de ternera), partidas en dos para que cada especie tenga SUS datos; el timo de vaca tiene 236 kcal y 20,35 g de grasa y el de ternera 101 y 3,07. Mas las 52 celdas cerradas contra BEDCA/CIQUAL/USDA en el orden de Bases.md, ninguna de las cuales era un numero mal copiado: las 52 eran CEROS. auditar_catalogo.py y auditar_fediaf.py re-ejecutados, pruebas_completas.py entero.
        ...
        "requerimientos_v2_final.json": "668ba90a84470fb3",
