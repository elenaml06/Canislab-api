...
def ratios_de_la_etapa(etapa, peso_adulto_esperado_kg=None):
    """Los RATIOS entre dos nutrientes que el libro recomienda al perro SANO.

    ⚠️ POR QUÉ EXISTE (18 de septiembre de 2026). Este fichero sabía de techos
    desde el 8 de septiembre y de suelos desde el 11, y le faltaba la tercera
    forma: **el cociente entre dos nutrientes**. Sin ella, una recomendación del
    libro que fuera un ratio no tenía dónde vivir -- exactamente el hueco que
    tuvo `patologias.json` hasta el 10 de septiembre, cuando el Ca:P de los dos
    urolitos de calcio llevaba dos días escrito con `aplicado_por_el_solver:
    false` porque el motor sabía de ratios Ca:P pero no tenía forma de que una
    PATOLOGÍA pidiera el suyo.

    El caso que lo pide es el **Ca:P del cachorro de raza grande**: la Tabla
    17-1 de SACN5 dice 1,5 y el motor aplica el 1,6 de FEDIAF. Hoy no cambiaría
    ni un menú --los reales van de 1,03 a 1,29-- y el propio libro se
    contradice, porque su Tabla 33-5 dice 2,0. Así que **la cifra no se
    enciende**: lo que se construye es el sitio donde puede vivir, medirse y
    auditarse, y encenderla es decisión de Elena y del nutricionista.

    Devuelve `{(numerador, denominador): {"min": x|None, "max": y|None}}`, la
    MISMA forma que `motor_completo.ratios_de_patologias`, para que el solver
    los combine en el mismo cajón y con el mismo `max()`/`min()` -- o sea que un
    ratio del libro solo puede APRETAR, nunca aflojar el de FEDIAF.

    Y con la misma puerta que los suelos: `aplicado_por_el_solver: false` deja
    la cifra escrita y auditada sin que el motor la aplique.
    """
    ficha = POR_ETAPA.get(etapa) or {}
    crudos = list(ficha.get("ratios") or [])
    grande = ficha.get("si_peso_adulto_esperado_supera_kg") or {}
    umbral = grande.get("umbral_kg")
    if (umbral is not None and peso_adulto_esperado_kg
            and peso_adulto_esperado_kg >= umbral):
        crudos += list(grande.get("ratios") or [])
    fuera = {}
    for r in crudos:
        if not r.get("aplicado_por_el_solver", True):
            continue
...
def ratios_escritos_de_la_etapa(etapa, peso_adulto_esperado_kg=None):
    """TODOS los ratios escritos para la etapa, apliquen o no.

    Hace falta para poder auditar los que están apagados: su conversión se
    rehace y su cita se comprueba igual que las de los que sí se aplican. Un
    número que deja de poder auditarse por estar apagado es un número que se
    pudre en silencio.
    """
    ficha = POR_ETAPA.get(etapa) or {}
    crudos = list(ficha.get("ratios") or [])
