...
# ============================================================
# BLOQUE 94 — LOS CONTADORES QUE NADIE EJECUTABA: NRC 2006 Y FASCETTI
# ============================================================
#
...
print("\n" + "=" * 60)
print("=== BLOQUE 94: los contadores de NRC 2006 y de Fascetti ===")
import subprocess as _sub94
for _quien94, _script94 in (("NRC 2006", "leer_nrc2006.py"),
                            ("Fascetti", "leer_fascetti.py")):
    if not _os_b18.path.exists(_os_b18.path.join(str(_raiz_b24), _script94)):
        fallos.append(f"BLOQUE94: falta `{_script94}`. El contador de {_quien94} tiene que "
                      f"existir y tiene que ejecutarse: sin él, «leído» vuelve a ser una "
                      f"palabra que nadie puede comprobar")
        continue
    _r94 = _sub94.run([sys.executable, _script94], capture_output=True, text=True,
                      cwd=str(_raiz_b24))
    _salida94 = (_r94.stdout or "") + (_r94.stderr or "")
    if _r94.returncode != 0:
        _cola94 = "\n      ".join(_salida94.strip().splitlines()[-8:])
        fallos.append(f"BLOQUE94: el contador de {_quien94} no cuadra:\n      {_cola94}")
    elif "no está el texto" in _salida94 or "no se puede contar" in _salida94:
        # Sin el repo de fuentes al lado no hay nada que contar, y se DICE en vez
        # de dar el bloque por bueno.
        print(f"  {_quien94}: no está el texto al lado, no se ha contado nada")
        continue
    for _l94 in _salida94.strip().splitlines():
        if _l94.startswith("-"):
            break
        if _l94.strip():
