...
# comprueba que el perro es de esa cuenta. Ver el BLOQUE 51, que es el que
...
# ============================================================
# BLOQUE 51 — LAS PUERTAS: QUIEN PUEDE PEDIR QUE
# ============================================================
#
# ⚠️ POR QUÉ EXISTE (11 septiembre). Los 50 bloques de arriba vigilan que
...
print("=== BLOQUE 51: las puertas -- quién puede pedir qué ===")

import stripe as _stripe_b51
import observabilidad as _obs_b51
import persistencia as _pers_b51
import tempfile as _tmp_b51, os as _os_b51
...
_CONSULTAS_B51 = []
_SUBS_B51 = [{"id": "s_victima", "status": "active", "customer": "cus_VICTIMA",
              "metadata": {"user_id": "otra-persona"}}]

def _buscar_b51(query=None, limit=None):
    _CONSULTAS_B51.append(query)
    return {"data": list(_SUBS_B51)}

_search_real_b51 = _stripe_b51.Subscription.search
_checkout_real_b51 = _stripe_b51.checkout.Session.create
_portal_real_b51 = _stripe_b51.billing_portal.Session.create
_clave_real_b51 = _stripe_b51.api_key
_CHECKOUTS_B51, _PORTALES_B51 = [], []

class _UrlB51:
    url = "https://portal.de.mentira"

_stripe_b51.Subscription.search = _buscar_b51
_stripe_b51.checkout.Session.create = lambda **kw: (_CHECKOUTS_B51.append(kw), _UrlB51())[1]
_stripe_b51.billing_portal.Session.create = lambda **kw: (_PORTALES_B51.append(kw), _UrlB51())[1]
_stripe_b51.api_key = "sk_test_b51"
...
if _api._user_id_limpio("  3f2b9c10-7a44-4e21-9b0e-51d2c8a77e01\n") != \
    ...
    fallos.append("BLOQUE51: un uid con espacios alrededor no se recorta y se rechaza. "
                  "Lo que va a la consulta es el recortado, así que recortar es seguro.")

try:
    for _mala_b51 in _INYECCIONES_B51:
        _CONSULTAS_B51.clear()
        _vivas_b51, _se_pudo_b51 = _api._suscripciones_vivas(_mala_b51)
        if _CONSULTAS_B51:
            fallos.append(
                f"BLOQUE51: con user_id={_mala_b51!r} se le ha preguntado a Stripe "
                f"igualmente ({_CONSULTAS_B51[0]!r}). Una comilla ahí dentro devuelve "
                f"las suscripciones de otra gente, y de ahí sale su portal de pago.")
        if _vivas_b51 or _se_pudo_b51:
            fallos.append(
                f"BLOQUE51: con user_id={_mala_b51!r} se ha contestado "
                f"({len(_vivas_b51)} vivas, se_pudo={_se_pudo_b51}). Tenía que ser "
    ...
    if not _CONSULTAS_B51:
        fallos.append("BLOQUE51: un UUID normal y corriente no llega a Stripe. La "
                      "validación se ha pasado de estricta y nadie podría suscribirse.")
    elif "3f2b9c10-7a44-4e21-9b0e-51d2c8a77e01" not in (_CONSULTAS_B51[0] or ""):
        fallos.append(f"BLOQUE51: la consulta a Stripe no lleva el id: {_CONSULTAS_B51[0]!r}")

    # el mismo filtro, por la puerta de /stripe/checkout
    for _mala_b51 in _INYECCIONES_B51[:3]:
        _CHECKOUTS_B51.clear()
        _r_b51 = _c.post("/stripe/checkout", json={"user_id": _mala_b51,
                                                   "email": "a@b.com", "plan": "mensual"})
        if _CHECKOUTS_B51:
            fallos.append(f"BLOQUE51: /stripe/checkout ha creado un cobro con "
                          f"user_id={_mala_b51!r}")
        if _r_b51.status_code != 400:
            fallos.append(f"BLOQUE51: /stripe/checkout con user_id={_mala_b51!r} ha "
                          f"devuelto {_r_b51.status_code}, esperaba 400")
    ...
    _PORTALES_B51.clear()
    _r_b51 = _c.post("/stripe/portal", json={"stripe_customer_id": "cus_VICTIMA"})
    if _r_b51.status_code != 401 or _PORTALES_B51:
        fallos.append(
            f"BLOQUE51: /stripe/portal ha abierto un portal con solo el id de cliente "
            f"(HTTP {_r_b51.status_code}, {len(_PORTALES_B51)} portales creados). Ese id no "
    ...
    _real_uid_b51 = _api._uid_del_token
    _api._uid_del_token = lambda t: "el-de-verdad" if t == "token-bueno" else None
    try:
        _PORTALES_B51.clear()
        _r_b51 = _c.post("/stripe/portal", json={"token_usuario": "token-inventado",
                                                 "stripe_customer_id": "cus_VICTIMA"})
        if _r_b51.status_code != 401 or _PORTALES_B51:
            fallos.append(f"BLOQUE51: /stripe/portal con un token que Supabase no reconoce "
                          f"ha devuelto {_r_b51.status_code}. Tenía que fallar cerrado.")

        # con un token bueno, el cliente sale de la suscripción de ESE uid,
        # nunca del cuerpo de la petición
        _SUBS_B51[:] = [{"id": "s_mia", "status": "active", "customer": "cus_MIA",
                         "metadata": {"user_id": "el-de-verdad"}}]
        _PORTALES_B51.clear()
        _r_b51 = _c.post("/stripe/portal", json={"token_usuario": "token-bueno",
                                                 "stripe_customer_id": "cus_VICTIMA"})
        if _r_b51.status_code != 200:
            fallos.append(f"BLOQUE51: /stripe/portal con un token bueno ha devuelto "
                          f"{_r_b51.status_code}; quien paga no puede gestionar su suscripción")
        elif not _PORTALES_B51 or _PORTALES_B51[0].get("customer") != "cus_MIA":
            fallos.append(
                f"BLOQUE51: el portal se ha abierto para "
                f"{(_PORTALES_B51 or [{}])[0].get('customer')!r}. Tenía que ser 'cus_MIA', el "
                f"de la suscripción de quien manda el token -- el `stripe_customer_id` del "
                f"cuerpo no puede decidir de quién es el portal.")
    finally:
        _api._uid_del_token = _real_uid_b51
        _SUBS_B51[:] = [{"id": "s_victima", "status": "active", "customer": "cus_VICTIMA",
                         "metadata": {"user_id": "otra-persona"}}]
    ...
    _r_b51 = _c.post("/stripe/checkout", json={"user_id": "otra-persona",
                                               "email": "a@b.com", "plan": "mensual"})
    _j_b51 = _r_b51.json()
    if not _j_b51.get("ya_suscrito"):
        fallos.append("BLOQUE51: /stripe/checkout no avisa de que ya hay suscripción")
    if _j_b51.get("url"):
        fallos.append(
            f"BLOQUE51: /stripe/checkout ha devuelto una URL ({_j_b51.get('url')!r}) a quien "
    ...
    _stripe_b51.Subscription.search = _search_real_b51
    _stripe_b51.checkout.Session.create = _checkout_real_b51
    _stripe_b51.billing_portal.Session.create = _portal_real_b51
    _stripe_b51.api_key = _clave_real_b51
...
_JWT_B51 = ("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"
            ".eyJzdWIiOiIxMjM0NTY3ODkwIiwicm9sZSI6ImF1dGhlbnRpY2F0ZWQifQ"
            ".dBjftJeZ4CVPmB92K27uhbUJU1p1r_wW1gFWFOEjXk")

_LIMPIO_B51 = _obs_b51._limpiar_datos({
    "token_usuario": _JWT_B51,
    "etapa_requisitos": "Adulto",
    "traza": f"datos = PeticionMenu(token_usuario='{_JWT_B51}', peso_perro_kg=20.0)",
    "anidado": [{"Authorization": f"Bearer {_JWT_B51}"}],
})
_TEXTO_B51 = json.dumps(_LIMPIO_B51, ensure_ascii=False)
if _JWT_B51 in _TEXTO_B51 or "eyJhbGciOi" in _TEXTO_B51:
    fallos.append(
        "BLOQUE51: el token de sesión de Supabase sigue saliendo entero hacia Sentry. "
...
# `token_usuario`.
_LIMPIO_NOMBRE_B51 = _obs_b51._limpiar_datos({"token_usuario": "una-sesion-cualquiera-12345"})
if "una-sesion-cualquiera-12345" in json.dumps(_LIMPIO_NOMBRE_B51):
    fallos.append(
        "BLOQUE51: un `token_usuario` que no tiene forma de JWT viaja entero a Sentry. "
        "Se compara el nombre del campo por TROZO, no por igualdad: la lista dice "
        "'token' y el campo se llama 'token_usuario'.")

if _LIMPIO_B51.get("etapa_requisitos") != "Adulto":
    fallos.append("BLOQUE51: la limpieza se ha llevado por delante datos del perro. "
                  "Sin ellos no se puede reproducir un fallo del motor.")
for _clave_b51 in ("sb_secret_AbCdEf123456", "sk_live_51AbCdEf123456", "whsec_AbCdEf123456"):
    if _clave_b51 in json.dumps(_obs_b51._limpiar_datos(f"algo salio mal con {_clave_b51}")):
        fallos.append(f"BLOQUE51: una clave de servicio ({_clave_b51[:12]}...) viaja a Sentry "
...
_db_b51 = _os_b51.path.join(_tmp_b51.mkdtemp(), "b51.db")
_pers_b51.crear_tablas(_db_b51)
_pid_mio_b51 = _pers_b51.guardar_perro({"nombre": "Mío", "usuario_id": "uid-mio"},
                                       ruta_db=_db_b51)
_pid_otro_b51 = _pers_b51.guardar_perro({"nombre": "De otro", "usuario_id": "uid-otro"},
                                        ruta_db=_db_b51)
# una fila de las de antes del 11 de septiembre: sin dueño que conste
_pid_huerfano_b51 = _pers_b51.guardar_perro({"nombre": "Sin dueño"}, ruta_db=_db_b51)

_real_menus_b51 = _pers_b51.obtener_menus
_real_dueno_b51 = _pers_b51.dueno_de
_real_uid_b51 = _api._uid_del_token
_pers_b51.obtener_menus = lambda _p, ruta_db=_db_b51: _real_menus_b51(_p, ruta_db=_db_b51)
_pers_b51.dueno_de = lambda _p, ruta_db=_db_b51: _real_dueno_b51(_p, ruta_db=_db_b51)
_api._uid_del_token = lambda t: {"token-mio": "uid-mio", "token-otro": "uid-otro"}.get(t)


def _estado_menus_b51(perro_id, token):
    try:
...
try:
    _pers_b51.guardar_menu(_pid_mio_b51, "uno", {"gramos": {"Pollo con piel (sin hueso)": 100},
                                                 "kcal_total": 200}, ruta_db=_db_b51)
    _casos_b51 = [
        (_pid_mio_b51, None, 401, "sin token ninguno"),
        (_pid_mio_b51, "token-inventado", 401, "con un token que Supabase no reconoce"),
        (_pid_mio_b51, "token-otro", 404, "con el token de OTRA persona"),
        (_pid_huerfano_b51, "token-mio", 404, "con un perro cuyo dueño no consta"),
        (_pid_otro_b51, "token-mio", 404, "pidiendo el perro de otro"),
        (_pid_mio_b51, "token-mio", 200, "siendo su dueño"),
    ]
    for _pid_b51, _tok_b51, _esperado_b51, _que_b51 in _casos_b51:
        _real_b51 = _estado_menus_b51(_pid_b51, _tok_b51)
        if _real_b51 != _esperado_b51:
            fallos.append(
                f"BLOQUE51: /perro/{{id}}/menus {_que_b51} ha devuelto {_real_b51}, "
                f"esperaba {_esperado_b51}. "
                + ("El historial de comida de un perro es de su dueño, y los ids van "
                   "1, 2, 3." if _esperado_b51 != 200
    ...
    _r_b51 = _c.get(f"/perro/{_pid_mio_b51}/menus",
                    headers={"X-Token-Usuario": "token-mio"})
    if _r_b51.status_code != 200:
        fallos.append(f"BLOQUE51: con la cabecera X-Token-Usuario, su dueño recibe "
                      f"{_r_b51.status_code}")
    if _c.get(f"/perro/{_pid_mio_b51}/menus").status_code != 401:
        fallos.append("BLOQUE51: sin cabecera ninguna, /perro/{id}/menus contesta igual")
    # el token NO puede colarse por la dirección: ahí se queda escrito
    if _c.get(f"/perro/{_pid_mio_b51}/menus?token_usuario=token-mio").status_code == 200:
        fallos.append("BLOQUE51: el token se acepta en la URL. Ahí acaba en los "
                      "registros de Render, en el historial y en el Referer.")
finally:
    _pers_b51.obtener_menus = _real_menus_b51
    _pers_b51.dueno_de = _real_dueno_b51
    _api._uid_del_token = _real_uid_b51
...
_BASE_B51 = {"gramos_por_alimento": {"Cuello de pollo": 300.0,
...
_r_b51 = _c.post("/pauta/firmar", json=_BASE_B51).json()

if not _r_b51.get("factible"):
    # no siempre hay ración verde a la primera; se prueba con lo que da el motor
    _menu_b51 = _c.post("/menu/v2", json={"nombres_alimentos": [], "modo": "automatico",
                                          "der_objetivo": 1100.0, "peso_perro_kg": 20.0,
                                          "etapa_requisitos": "Adulto"}).json()
    if _menu_b51.get("factible"):
        _BASE_B51["gramos_por_alimento"] = _menu_b51["menu"]
        _r_b51 = _c.post("/pauta/firmar", json=_BASE_B51).json()

if not _r_b51.get("factible"):
    fallos.append("BLOQUE51: no se ha podido firmar ninguna pauta para probar el sello. "
                  "Sin documento no se comprueba nada -- mira si /pauta/firmar sigue en pie.")
else:
    _doc_b51 = _r_b51["documento"]
    if not _doc_b51.get("sello_con_clave"):
        fallos.append("BLOQUE51: el documento firmado no dice que su sello lleve clave. "
                      "Dentro de un año, quien lo lea no podrá saberlo.")
    if not _c.post("/pauta/comprobar", json=_doc_b51).json().get("coincide"):
        fallos.append("BLOQUE51: un documento recién firmado no se reconoce a sí mismo.")

    # LA FALSIFICACIÓN: se cambia lo que importa y se vuelve a sellar con la
    # receta que está escrita en main.py, que es pública.
    import hashlib as _hash_b51
    _falso_b51 = json.loads(json.dumps(_doc_b51))
    _falso_b51["firmante"] = {"nombre": "Quien sea", "num_colegiado": "COL-INVENTADO"}
    _falso_b51["menu"] = {k: v * 2 for k, v in _falso_b51["menu"].items()}
    _falso_b51["sello"] = _api._sello_de(_falso_b51, clave="")  # sin clave: la receta pública
    _v_b51 = _c.post("/pauta/comprobar", json=_falso_b51).json()
    if _v_b51.get("coincide"):
        fallos.append(
            "BLOQUE51: se ha dado por AUTÉNTICA una pauta reescrita -- otro menú, otro "
    ...
    if _v_b51.get("sello_con_clave"):
        fallos.append("BLOQUE51: un sello sin clave se ha presentado como que la lleva.")

    # y cambiar un gramo de un documento bien sellado tiene que saltar igual
    _tocado_b51 = json.loads(json.dumps(_doc_b51))
    _primero_b51 = sorted(_tocado_b51["menu"])[0]
    _tocado_b51["menu"][_primero_b51] = _tocado_b51["menu"][_primero_b51] + 7
    if _c.post("/pauta/comprobar", json=_tocado_b51).json().get("coincide"):
        fallos.append("BLOQUE51: se han movido 7 g de un documento firmado y el sello "
                      "sigue cuadrando.")

# sin la clave puesta no se firma: se prefiere no dar el papel a darlo sin que valga
_guardada_b51 = _os_b51.environ.pop("SELLO_SECRETO", None)
try:
    _r_b51 = _c.post("/pauta/firmar", json=_BASE_B51)
    if _r_b51.status_code != 503:
        fallos.append(f"BLOQUE51: sin SELLO_SECRETO se ha firmado igual "
                      f"(HTTP {_r_b51.status_code}). Un sello sin clave lo recalcula "
                      f"cualquiera; firmar con él es entregar un papel que no prueba nada.")
finally:
    if _guardada_b51 is not None:
        _os_b51.environ["SELLO_SECRETO"] = _guardada_b51
...
for _idx_b51 in (-1, -5, 5, 99):
    _r_b51 = _c.post("/der", json={"peso_actual_kg": 20.0, "etapa": "adulto",
                                   "actividad_idx": _idx_b51, "esterilizado": False})
    if _r_b51.status_code != 400:
        fallos.append(f"BLOQUE51: /der con actividad_idx={_idx_b51} ha devuelto "
                      f"{_r_b51.status_code} en vez de 400. Un índice negativo cuenta "
...
_ders_b51 = {}
for _idx_b51 in range(5):
    _r_b51 = _c.post("/der", json={"peso_actual_kg": 20.0, "etapa": "adulto",
                                   "actividad_idx": _idx_b51, "esterilizado": False})
    if _r_b51.status_code != 200:
        fallos.append(f"BLOQUE51: /der con actividad_idx={_idx_b51} (que es válido) "
                      f"ha devuelto {_r_b51.status_code}: {_r_b51.text[:200]}")
        continue
    _ders_b51[_idx_b51] = (_r_b51.json() or {}).get("der")
    if not _ders_b51[_idx_b51]:
        fallos.append(f"BLOQUE51: /der con actividad_idx={_idx_b51} ha contestado 200 "
                      f"pero sin DER: {_r_b51.text[:200]}")
if len(_ders_b51) == 5 and not (_ders_b51[0] < _ders_b51[2] < _ders_b51[4]):
    fallos.append(f"BLOQUE51: el DER no sube con la actividad ({_ders_b51}). O el índice "
...
_gordo_b51 = _c.post("/der", json={"peso_actual_kg": 26.0, "peso_objetivo_kg": 20.0,
                                   "etapa": "adulto", "actividad_idx": 1,
                                   "esterilizado": False})
if _gordo_b51.status_code != 200:
    fallos.append(f"BLOQUE51: /der con peso objetivo ha devuelto {_gordo_b51.status_code}")
elif _ders_b51.get(1) and (_gordo_b51.json() or {}).get("der", 0) > _ders_b51[1] * 1.2:
    fallos.append(f"BLOQUE51: un perro de 26 kg con objetivo 20 recibe "
                  f"{_gordo_b51.json().get('der')} kcal, más que uno que ya pesa 20 "
                  f"({_ders_b51[1]}). El peso objetivo no está llegando al cálculo.")
...
if "*" in _api.ORIGENES_PERMITIDOS:
    fallos.append("BLOQUE51: el CORS vuelve a estar abierto a cualquier origen.")
_r_b51 = _c.get("/", headers={"Origin": "https://sitio-cualquiera.example"})
if _r_b51.headers.get("access-control-allow-origin"):
    fallos.append(f"BLOQUE51: un origen desconocido recibe permiso de CORS "
                  f"({_r_b51.headers.get('access-control-allow-origin')!r}).")
_r_b51 = _c.get("/", headers={"Origin": "https://rawku.app"})
if _r_b51.headers.get("access-control-allow-origin") != "https://rawku.app":
    fallos.append("BLOQUE51: rawku.app NO recibe permiso de CORS. La app se quedaría "
