...
for etapa, peso in CASOS_ETAPA:
    ...
    if not ok:
        # un solo intento puede fallar por la aleatoriedad -- reintentar
        # una vez antes de contarlo como fallo real, igual que hace el
        # servidor de verdad
        ok, g = resolver(der, etapa, al, req, peso, dosis_maxima_fabricante,
                         margenes_categoria=MARGENES, max_suplementos=2)
    if not ok:
        fallos.append(f"BLOQUE1 {etapa} {peso}kg: NO FACTIBLE ni al reintentar")
        continue
