...
| SACN5 | **leídos enteros sus 70 capítulos de perro y gato** (10-11 sep) ⚠️ **el libro tiene 72** (12-sep) · el repaso punto por punto está **repartido en tres sitios** | ⚠️ **CORREGIDA ESTA FILA EL 12-sep.** Decía «leída, pero NO con este método», y eso es falso en la mitad que más cuesta: `LECTURA_SACN5_INTEGRA.md` recorre los **70 capítulos de la primera línea a la última** y lo dice al cerrar. Lo que sí es verdad es la otra mitad: el documento remite las decisiones a `HALLAZGOS_SACN5_10SEP.md`, a `HALLAZGOS_SACN5_11SEP.md` y a los pendientes por tema, así que **el veredicto de cada punto no está en un solo sitio**. Eso es lo que le falta, y no la lectura. || ⚠️ **CORREGIDO EL 12-sep**: aquí ponía «los 70 capítulos» y el libro tiene **72**. Los dos que faltan en nuestra copia son el **71 (reptiles)** y el **72 (aves)**, comprobado bajándolos del Mark Morris Institute, que los sirve gratis. Para el perro no cambian nada y por eso no lo vio nadie, pero «leída entera» tiene que significar entera o no significa nada. || Y de paso: el editor ofrece los 72 capítulos y **ningún apéndice**, así que la tabla de peso ideal por raza -- que está en el apéndice de la **4.ª edición**, pp. 1037-1046 -- no se puede conseguir por ahí. Ojo con esa cita: son páginas de la CUARTA edición, y en la quinta esas mismas páginas son el capítulo 53, la dilatación-torsión gástrica de Davenport.
...
| `TRABAJO_RAWKU/` — la base de datos de purinas | ⚠️ **aplicada y sin registrar hasta el 12-sep** | ⚠️ **AÑADIDA ESTA FILA EL 12-sep.** Es una carpeta entera del repo de fuentes que **no aparecía ni una vez en esta tabla**, y dentro hay una fuente primaria de verdad: `fuentes_datos/PURINEDATABASEANDDATASOURCES2025.xlsx`, que su propio LÉEME llama «la base de datos de purinas, fuente primaria de verdad». Sus datos **ya están en el catálogo** — 101 fichas con `purinas_bases` y su `purinas_fuente` ficha a ficha —, o sea que está aplicada; lo que faltaba era el registro. Aplicar sin registrar es medio método: nadie puede saber qué se miró y qué no. || Y quedan dos cosas suyas **sin revisar**, marcadas así por su propio LÉEME: `investigacion/especias.html` y `investigacion/patologias.html`, que su propio LÉEME manda tratar como borrador hasta que alguien lo compruebe. Lo tercero que hay dentro, `entregas/AUDITORIA_PATOLOGIAS.md` con las 13 condiciones clínicas sin perfil, sí está seguido: es la P-07 |
...
| ACVIM (Keene 2019) | **leído entero (12-sep)** | Igual: primero se buscó y luego se leyó. Lo que solo aparece leyendo va marcado abajo | Abajo. Cierra P-35 —antes P-10, renumerada el 12-sep por estar repetida— …el límite del sodio existe y es clínico, no una cifra— y saca un hallazgo nuevo: el motor aplica al cardíaco con renal justo la dieta baja en proteína que el consenso dice que hay que evitar |
...
| Today's Veterinary Practice — Cook & Atiee, oxalato cálcico | **leída entera (12-sep)** | Es el ÚNICO artículo de esta revista que hay en el repo, y está completo. Al leerlo salió lo de las purinas, que ningún `grep` mío habría buscado. Aguanta entera: todo lo suyo ya estaba aplicado |
| **Cavanaugh 2020 — *Veterinary Practice News*** | ⚠️ **no está en el repo** | ⚠️ **CORREGIDA ESTA FILA EL 12-sep, y la corrección es de NOMBRE.** Esto vivía dentro de la fila de arriba, escrito como que el que da el sodio cardíaco por estadio ACVIM sigue sin estar en el repo. Son **dos revistas distintas** con el nombre casi igual — *Today's Veterinary Practice* y *Veterinary Practice News* —, y meterlas en la misma fila hacía que el registro dijera «leída entera» y «no está» de la misma cosa. Lo cazó Elena leyéndolo. || Es «Understanding nutrition in dogs with degenerative mitral valve disease», de Cavanaugh (DACVIM cardiología), 6-jul-2020, y de ahí sale **el rango numérico del sodio de CUATRO claves cardíacas**: `cardiopatia` (739), `cardiopatia_b2`, `cardiopatia_c` (625) y `cardiopatia_d` (480). O sea que el número que decide la sal de un perro con el corazón enfermo viene de un artículo que no se puede abrir aquí para comprobarlo. El consenso ACVIM (Keene 2019) sí está y sí se leyó entero, pero **da el criterio clínico y no el rango por estadio**: eso es exactamente lo que aporta este |
...
### P-35 queda contestada, y la respuesta no es un número (era P-10, renumerada el 12-sep por estar repetida)
...
| Lo que dice | Decisión |
|---|---|
| *«Legal maxima in EU legislation are expressed on 12% moisture content and they do not account for energy density. Therefore in these guidelines they are only provided on a dry matter basis»* y *«These conversions assume an energy density of 16.7 kJ (4.0 kcal) ME/g DM. For foods with energy densities different from this value, the recommendations should be corrected for energy density»* | ⚠️ **PENDIENTE DE DECIDIR, y aquí ponía lo contrario hasta el 14 de septiembre.** El 11 de septiembre esta casilla decía «medido, y hoy es benigno por coincidencia: una ración BARF mide 4040 kcal/kg MS, un 1 % de diferencia» — y declaraba, en la misma frase, por qué no podía saberlo: «el catálogo **no guarda la materia seca de ninguna de sus 163 fichas**, así que eso no se puede recalcular, solo estimar». Era una **estimación presentada como medida**, y estaba mal por un 29 %. Cerrada la humedad de 144 fichas el 14 de septiembre, la medida de verdad sobre los 216 menús del catálogo es **5,20 kcal/g de materia seca** (4,05-6,18), no 4,0: una ración BARF es proteína y grasa sin almidón, sin fibra y sin ceniza, que es lo que abarata la densidad de un pienso. O sea que el ×2,5 va un **23 % flojo**, y lo usan **135 cifras** del motor — las 94 de `patologias.json`, las 24 de `recomendaciones_libro.json`, las 4 de `requisitos_condicionales.json` y **los 13 máximos de FEDIAF que solo se publican en base materia seca**, siete de ellos legales. Medido lo que costaría aplicarlo: **8 de 8** perros de referencia siguen saliendo con menú. La decisión está en **P-38** de `PREGUNTAS_ABIERTAS.md` y la vigila el BLOQUE 111 |
